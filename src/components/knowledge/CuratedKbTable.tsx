...
export function CuratedKbTable() {
  ...
  const patch = async (id: string, updates: Record<string, unknown>, successMsg: string) => {
    setLoadingId(id);
    updateStatus.mutate(
      { id, updates },
      {
  ...
  if (!rows || rows.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
        <p className="text-sm">No curated entries yet</p>
        <p className="text-xs mt-1">Create KB items from the Escalation detail page.</p>
      </div>
    );
  }
    ...
    if (row.status === "PROPOSED") {
      ...
      return (
        <div className="flex gap-1.5">
          ...
          <Button size="sm" variant="outline" className="text-xs h-7 gap-1" disabled={busy}
            onClick={(e) => { e.stopPropagation(); patch(row.id, { status: "DRAFT" }, "Rejected to Draft"); }}>
            {spinner} Reject
          </Button>
        </div>
      );
